...
def fossologywaitextracted(d, bb, info):
    ...
    time.sleep(10)
    while i < 24:
        upload_id = has_upload(d, tar_name, info['folder_id'])
        bb.note("spdx: upload_id :" + str(upload_id))
        if upload_id:
            return
        i += 1
...
def has_upload(d, tar_file, folder_id):
    ...
    if len(upload_output) == 0:
        ...
        return False
    bb.note("upload_output[0][uploadname] = " + upload_output[0]["uploadname"])
    for i in range(0, len(upload_output)):
        if upload_output[i]["uploadname"] == file_name and str(upload_output[i]["folderid"]) == str(folder_id):
            bb.note("Found " + file_name + " on fossology server \"Software Repository\" folder. Will skip upload.")
