...
addtask do_spdx_upload before do_spdx after do_patch

python do_spdx_upload () {
    ...
    bb.note(' Upload source code to fossology server. ')
    if not os.path.isfile(tar_name):
        bb.warn(info['pn'] + "has no source, do nothing")
        return
    folder_id = get_folder_id(d)
    if invoke_upload(d, tar_name, folder_id) == False:
        bb.warn(pn + ": Get spdx file fail, please check fossology server.")
...
python do_spdx() {
    ...
    tar_name = spdx_get_tarall_name(d, 'patched', spdx_outdir)
    ...
    ## Get spdx file
    bb.note(' Begin to start analyse to get spdx. ')
    folder_id = get_folder_id(d)
    if invoke_get_spdx(d, tar_name, sstatefile, folder_id) == False:
...
def invoke_upload(d, tar_file, folder_id):
    import os
    import time
    i = 0
...
def invoke_get_spdx(d, tar_file, spdx_file, folder_id):
    import os
    import time
    i = 0
    ...
    upload_id = has_upload(d, tar_file, folder_id)
    if upload_id == False:
        bb.error("This OSS has not been scanned. So upload it to fossology server.")
        return False
